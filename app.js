...
function review(){
  ...
  ${all.length?`<div class="flex" style="margin-bottom:24px">${button('복습 20문항 시작 ↻','start-review')}${button('오답만','start-wrong','','secondary')}${button('헷갈림만','start-unsure','','secondary')}</div><div class="panel">${ranked(all).slice(0,30).map(q=>{const p=progress(q.id);return `<div class="topic-row" style="grid-template-columns:65px 1fr auto"><span class="pill ${p.wrong?'red':''}">${p.wrong?'오답':p.unsure?'헷갈림':'복습 도래'}</span><div><b>${esc(q.prompt)}</b><p class="small">${TOPIC[q.topic].name} · ${q.authored?'자체 제작 연습':`${q.examDate?'원본 해설집':'정리본'} ${q.page}쪽`} · ${p.attempts}회 풀이</p></div>${button('풀기','start-one',`data-id="${q.id}"`,'small secondary')}</div>`;}).join('')}${all.length>30?`<p class="small muted" style="margin-top:12px">상위 30개를 표시했습니다. 복습 시작 시 전체 ${all.length}문항에서 구성합니다.</p>`:''}</div>`:`<div class="empty"><h2>지금 복습할 문제가 없습니다.</h2><p>${next?`${dateLabel(next.due)}에 다음 복습이 열립니다.`:'문제를 풀면 오답과 복습 일정이 여기에 모입니다.'}</p><a class="btn" href="#learn">새 문제 풀러 가기 →</a></div>`}`;
}
...
document.addEventListener('click',event=>{
  ...
  switch(action){
    ...
    case 'start-source-curriculum':startSourceCurriculum(el.dataset.source,el.dataset.topic);break;
