...
function validateState(value){
  ...
  if(s){
    ...
    if(s.focus?.course&&((s.focus.courseVersion!==undefined&&s.focus.courseVersion!==2)||!s.courseRead||typeof s.courseRead!=='object'||Array.isArray(s.courseRead)||Object.entries(s.courseRead).some(([k,v])=>!/^\d+$/.test(k)||Number(k)>=courseUnits(s).length||typeof v!=='boolean')||s.items.some(i=>!Number.isInteger(i.lesson)||i.lesson<0||i.lesson>=courseUnits(s).length)))return null;
    if(s.focus!==undefined&&(!s.focus||s.mode!=='practice'||!Array.isArray(s.focus.conceptIds)||!s.focus.conceptIds.length||s.focus.conceptIds.length>(s.focus.course?10:6)||!s.focus.conceptIds.every(id=>CONCEPTS.some(c=>c.id===id))))return null;
